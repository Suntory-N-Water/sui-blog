...
Cloudflare D1とR2
        ↓
画像ブロックの正規化（`pnpm normalize:media`）
        ↓
公開ページ、RSS、llms.txt、Markdown、OGP画像
```

初期データ登録の直後に`pnpm normalize:media`を実行する。seedの画像ブロックはメディアの登録前に書かれているため、D1へ入った時点では本文中の画像がEmDashのメディアIDを指しておらず、そのままでは本文の画像が表示されない。この処理はD1のメディア一覧から本文の画像ブロックに対応する行を探し、参照をメディアIDへ書き換える。
...
10.1から順に実施する。10.1が終わるまで10.2以降に着手しない。

### 10.1 記事本文が表示されていることの確認

移行した記事の本文が公開ページに表示されない状態は、この移行で最も重大な失敗である。そのため、URL構成やレイアウトの調整より先にこの確認を行う。

現時点で確認できているのはHTTPステータスと記事件数だけであり、本文がHTMLとして出力されているかは未確認である。

1. 全130記事の記事詳細ページを取得し、本文の段落、見出し、リスト、リンク、強調がHTMLとして出力されていることを確認する。
2. ブロック種別ごとに代表記事を選び、表示を確認する。対象はコードブロック、表、引用、`htmlBlock`、脚注、`mermaid`指定のコードブロックとする。
3. 元のMarkdownと記事詳細ページの本文を比較し、段落数と見出しの並びが一致することを確認する。
4. 画像ブロックのうち`asset`が空の52箇所が、読み込めない`<img>`や表示崩れを起こさないことを確認する。取得できなかった画像64件そのものは過去記事のため対応しない。
5. 記事一覧、タグ別一覧、検索結果で、タイトル、概要、タグ、公開日が表示されることを確認する。

完了条件: 全記事で本文が表示され、表示されない記事と崩れるブロック種別を一覧できる。

### 10.2 URL構成の見直し

現在のURL構成は§7のとおり。変更したい箇所を洗い出し、変更前と変更後の対応表を作ってから実施する。§2のとおり旧URLへの転送は作らない。

完了条件: 変更後のURLで公開面が動作し、§7の記述が実装と一致する。

### 10.3 レイアウトの調整

記事詳細の3カラム構成、目次、タグの表示、記事一覧の並びなど、表示上の調整を行う。`AGENTS.md`のとおり、変更は`src/styles/theme.css`のトークン上書きで行い、`src/styles/tokens.css`と`src/layouts/Base.astro`は変更しない。

完了条件: 調整後の表示をローカルで確認でき、公式テーマの構成を独自実装で置き換えていない。

### 10.4 リモート環境への公開

§8.6のとおり、明示的な承認後に行う。10.1から10.3が完了してから着手する。
