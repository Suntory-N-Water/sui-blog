# Cloudflareリモートデータの退避手順

この文書は、Cloudflare上に既存データがある状態で初期データ登録、削除、再作成を行う場合の条件付き手順である。今回のローカル移行や、空のリモート環境への初回登録には適用しない。退避はデータを別の場所へ移行する処理ではなく、失敗時に元へ戻すためのコピーである。

作業前に明示的な承認を得る。認証情報、アカウントID、バケットの内容をリポジトリへ保存しない。
