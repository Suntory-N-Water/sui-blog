...
### 時代背景→一般課題→自分の経験の3段構成

いきなり自分の話に入らず、読者の共通認識を先に置く。「技術の時代的な背景」→「そこにある一般的な課題」→「自分も同じだった」の順番で共感の土台を作ってから本題に入る。
...
初出の専門用語・略語には脚注 (`[^用語]`) を検討する。「この単語を知らない読者が読んだらどう感じるか」を基準にする。知っている読者は本文だけ読めばよく、知らない読者は脚注で補えるという二層構造にする。
