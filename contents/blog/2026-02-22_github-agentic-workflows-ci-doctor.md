...
`tools` の `cache-memory: true` を設定すると、エージェントがラン間で診断結果をキャッシュファイルに永続化できます。`skip-if-match` を通過したケース (PR はクローズ済みだが同じ原因の失敗が再発した場合など) でも、前回の診断結果と比較して重複対応を防止できます。`skip-if-match` がフロントマターレベルの第 1 層、`cache-memory` が AI 実行レベルの第 2 層として、多層的に冪等性を担保しています。
