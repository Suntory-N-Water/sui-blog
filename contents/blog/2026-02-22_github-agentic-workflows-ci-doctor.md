...
私が運用するリポジトリでも同じ問題を抱えていました。ドキュメント取得や CHANGELOG 処理のワークフローが定期的に失敗し、Issue 作成は自動化していても、原因調査と修正は毎回手動です。今回は Agentic Workflows で CI が失敗したとき自動で診断・修正するワークフローを構築し、その設計・セキュリティ設計・検証結果を報告します。
...
従来の GitHub Actions は YAML に「このコマンドを実行して、成功したら次のコマンドを実行する」という手順を事前に決めて書くしくみです。あらかじめ決めた手順どおりにしか動きません。一方で gh-aw は、Markdown に自然言語で「何をしてほしいか」を書きます。AI エージェントがその指示を読み取り、自分で判断しながらタスクを実行します。
