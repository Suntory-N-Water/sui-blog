...
description: HTML から決まった項目を取り出すコードは、対象のページの構造が変わると何も取れなくなります。CSS セレクターが一致しなくなった時点で候補をコード側で作り、どれが見出しかを選ぶ判断だけを TypeSafe の Jev に送る仕組みを作りました。架空のニュースサイトで一覧ページと記事ページで動かした結果、選ばれたセレクターを採用してよいかを前回の件数で確かめた結果と、1 回あたりの入力とコストをまとめます。
icon: 🃏
...
取得は失敗しないので、0 件になったことを選び直しの開始条件にする作りでは、この変化のときに選び直しが始まりません。前回の件数も保存しておき、件数のずれも開始条件に加えました。件数を `state` に入れると選択は悪くなりましたが、それはモデルへ渡した場合の話で、選び直しを始めるかどうかをコード側で判断するのには使えます。
...
ここまでの表の「正しい」は、私が取れた値を目で見て付けた判定です。コードの側は、Jev が返したグループのセレクターをそのまま書き戻していました。
