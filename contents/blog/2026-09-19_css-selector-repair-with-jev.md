---
title: CSS セレクターの選び直しを、Jev に 1 リクエスト $0.0005 で任せてみる
slug: css-selector-repair-with-jev
date: 2026-09-19
modified_time: 2026-09-20
description: HTML から決まった項目を取り出すコードは、対象のページの構造が変わると何も取れなくなります。CSS セレクターが一致しなくなった時点で候補をコード側で作り、どれが見出しかを選ぶ判断だけを TypeSafe の Jev に送る仕組みを作りました。架空のニュースサイトの一覧ページと記事ページで動かし、選ばれたセレクターを採用してよいかを前回の件数で確かめた結果と、1 回あたりの入力とコストをまとめます。
icon: 🃏
...
取り出し方として広く使われているのは、HTML をそのまま生成モデルに渡して、必要な項目を書き出してもらう書き方です。渡すのは HTML そのものなので、ページの構造が変わっても取り出しに失敗しません。その代わり、1 ページ分の HTML は 12 万バイトを超えることもあり、入力トークンがページ数にそのまま比例して増えます。
...
セレクターが一致しなくなったときだけ、生成モデルに HTML 全体を渡して書き直してもらう案もあります。ただし入力トークンは渡した HTML の大きさに比例します。今回題材にしたページで数えると、HTML 1 ページ分は 5 万トークンを超えました。取り出したい項目は決まっていて、必要なのはその場所を選ぶ判断だけです。それでも、選び直しのたびに HTML 1 ページ分をそのまま送ることになります。
...
リクエストに入れるのは `state`、`model`、`questions` の 3 つです。`state` は判断の前提になる情報で、文字列でも JSON のオブジェクトでも配列でも渡せます。`questions` は質問 ID をキーにしたマップで、同じ `state` に対して複数の質問を入れられます。
...
flowchart TB
    ...
    E --> F["選ばれたグループのセレクターで取り直す"]
    F --> G{"件数が前回と一致するか"}
    G -->|一致する| H["コードのセレクターを置き換える"]
    G -->|一致しない| I["採用せず、人が確認する"]
```
...
`MAX_CANDIDATES` を 254 にして、1 つの質問に置ける選択肢 255 個のうち 1 つを後述する `NONE` に空けています。この一覧ページでは候補が 30 グループにしかならず、上限を超える場面は来ませんでした。
...
題材にしたページは styled-components で作られています。styled-components は、CSS を JavaScript のコードの中に書き、その内容から class 名を自動で作って要素に付けるライブラリです。書き手が class 名を決めるタイミングはありません。`jYbJVt` はスタイルの中身から作られた class 名で、`sc-1t7ra5j-7` はコンポーネントを識別するための id です。後者は、コンポーネントを定義したファイルから作られたハッシュと、そのファイルの中での連番でできています。スタイルの中身やコンポーネントの並びが変われば、どちらも別の文字列になります。

つまり、選び直した結果に入っているのは、セレクターが一致しなくなった原因そのものです。そこで、候補を作る段階で、生成された class 名を取り除きます。styled-components が同じ要素に付けるのは、`sc-<ハッシュ>-<連番>` の形の class と、スタイルから生成した class の 2 つです。片方を見つけたら、その要素の class はすべて取り除きます。
...
サイト側のビルドはできないため、取得済みの HTML を書き換えて 3 パターン作りました。内容は次のとおりです。
...
class 名の置き換えには、seed を固定した乱数を使いました。seed が同じなら、同じ HTML からは毎回同じ class 名ができます。styled-components が名前をどう作るかまでは真似ておらず、再ビルドで名前がすべて別の文字列になるという結果だけを再現しています。
...
このデモ用ページは、題材にしたページと同じセレクターで同じ件数が取れるように作ってあります。`a > div > div > div > p` で 13 件、`div > div > div > ul > li > a` で 8 件です。どちらのグループも、テキストは記事の見出しそのものです。セレクター、出現回数、テキストの例だけを渡された側に、どちらが目当ての 13 件かは決められません。
...
件数だけの行と、値の例 1 件の行を見比べます。件数だけを渡した 9 / 15 は、何も渡さない 11 / 15 より下がりました。13 件という数だけで判断すると、記事カード全体を指す `a > div > div` や、その 1 つ上の親要素が選ばれます。どれも 13 件あるので条件は満たしますが、取れるのはカードの中身をつなげた文字列です。
...
取得は失敗しないので、0 件になったことを選び直しの開始条件にする作りでは、この変化のときに選び直しが始まりません。前回の件数も保存しておき、件数のずれも開始条件に加えました。件数を `state` に入れると選択は悪くなりましたが、それはモデルへ渡した場合の話です。選び直しを始めるかどうかをコード側で判断するのには使えます。
...
`article` の子に親の `div` を 1 段足し、`article_body` を `articleBody` に変え、`time` を `span` にする変更を記事ページに加えたうえで、別の記事の前回値を渡しました。
...
選択自体は 3 条件とも同じでしたが、前回値を入れたほうが confidence は下がりました。一致しない例を渡すと、確率の分かれる先が 1 つ増えます。「これは別の記事の値なので、値の種類だけを見てほしい」と伝える 1 文を足しても、元の値までは戻りませんでした。
...
ここまでの表の「正しい」は、取れた値を私が目で見て付けた判定です。コードの側は、Jev が返したグループのセレクターをそのまま書き戻していました。

前回の取得件数は、13 件が 26 件になる変化を開始条件に加えるためにすでに保存してあります。同じ値で、取れた件数が前回と一致するかを確かめます。

```python
def adopt(document, selector, previous_count):
    """件数が前回と違うセレクターは、取得できていても前回とは別の列を指している"""
    found = matches(document, selector)
    return len(found) == previous_count, found
```

選び直した直後にこれを呼び出します。1 つ目の戻り値が偽なら、セレクターを書き戻さず、その項目を未解決のまま残して人が確認します。2 つ目の戻り値は、そのまま取得結果として使えます。

### 3 種類の誤選択を採用しないか

ここまでに出てきた誤選択 3 種類について、採用しないで済むかを数えました。一覧ページの 3 パターンを 3 回ずつ、記事ページの変更前と変更後を 3 回ずつ、ここまでの表とは別に実行しています。選択は 36 回で、そのうち誤りは 8 回でした。前回の値は `state` に入れず、件数の確認だけに使っています。

| 誤選択 | 回数 | 前回の件数 | 取れた件数 | 採用 |
|---|---:|---:|---:|---|
| 一覧ページの見出しに 8 件のグループが選ばれた | 2 | 13 | 8 | しない |
| 記事ページの掲載日時に 10 件のグループが選ばれた | 3 | 1 | 10 | しない |
| 記事ページの配信元にサイト名が選ばれた | 3 | 1 | 1 | する |

誤ったまま採用されるのは、配信元にサイト名が選ばれた 3 回です。サイト名も媒体名もページに 1 つずつしかないため、前回の件数と取れた件数がどちらも 1 になります。採用しなかったのは誤り 8 回のうち 5 回、種類では 3 種類のうち 2 種類です。

### 正しい選択を採用しなかった 6 回

同じ 36 回のうち、正しい選択なのに採用しなかった回が 6 回ありました。すべて記事ページの本文です。

| 正しい選択 | 回数 | 前回の件数 | 取れた件数 | 採用 |
|---|---:|---:|---:|---|
| 記事ページの本文 | 6 | 2 | 1 | しない |

前回の件数 2 は、別の記事から取った段落数です。題材にした記事の本文は 1 段落なので、正しく選べていても一致しません。一覧ページの 15 回では、正しい選択を採用しなかった回はありませんでした。

件数の確認が使えるのは、ページが変わっても件数が変わらない項目に限られます。一覧ページの見出し・配信元・掲載時刻は、記事が 13 件並ぶかぎり件数が変わりません。記事ページでは、タイトル・配信元・日時のように 1 件と決まっている項目には使えますが、本文には使えません。

### confidence と正誤の関係

同じ 36 回の confidence を見ると、正しい選択と誤った選択が同じ範囲に並びます。記事ページの変更前では、正しい掲載日時が 0.35 / 0.41 / 0.48、誤った配信元が 0.45 / 0.47 / 0.49 でした。変更後では、誤った掲載日時が 0.71 / 0.72 / 0.74 で、正しい本文の 0.80 / 0.83 / 0.83 に近い値です。

confidence が下がるのは、似た候補が複数あるときです。似た候補のうちどれが正しいかは、その値からは分かりません。人が確認するかどうかは、confidence ではなく件数の一致で決めます。
...
class 名を変更した HTML に前回値なしで 3 回聞いたうち、1 回は 8 件のグループが選ばれました。選択がばらついた前回値なしの見出しの confidence は 0.26 から 0.44、3 回とも同じ選択になった前回値ありは 0.92 から 0.96 です。confidence から読み取れるのは、同じ入力で選択がばらつくかどうかまでです。そのばらついた選択が正しいかどうかは、採用の前の件数の確認で判断します。
...
- 候補に無い値は選べないため、日時のように隣の要素と同じ構造になった値は、質問の書き方では直せない
- 選び直したセレクターで取れた件数が前回の件数と一致するときだけ採用すると、36 回の選択で起きた誤選択 8 回のうち 5 回を採用しない。採用してしまうのは、配信元にサイト名が選ばれた場合のように、前回も今回も 1 件になる誤選択である
- 件数の確認が使えるのは、ページが変わっても件数が変わらない項目に限る。記事ごとに段落数が変わる本文では、正しい選択も 6 回とも採用しなかった
- confidence から読み取れるのは、同じ入力で選択がばらつくかどうかまでである。記事ページの変更前では正しい掲載日時が 0.35 から 0.48、誤った配信元が 0.45 から 0.49 と同じ範囲に並び、正誤は分けられない
