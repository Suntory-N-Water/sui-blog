...
Claude Code には Explore、Plan、general-purpose といった built-in の subagent があり、用途別に呼び分けて使えます。私もここ数か月、Explore で読み、Plan で設計、general-purpose で調査と、built-in をそのまま呼び分けて使ってきました。

カスタム agent を作るのは、正直なところ気が進みませんでした。description が機能するか確信が持てないですし、いったん作れば保守も発生します。「既存ので動いているし、まあいいか」と先送りにしていました。これは怠慢というより、コストを払う動機が弱かっただけだと思っています。

[こちらの記事](https://suntory-n-water.com/blog/claude-code-usage-otel-cloudflare-d1) で作った OTEL → Cloudflare D1 の収集基盤に 1 か月分のデータが溜まったので眺めてみたら、`general-purpose` は 12 件中 11 件が Opus で走っていました。軽いディレクトリ調査でも、親が Opus なら子も Opus です。一方で `Explore` は 15 件すべて Haiku 4.5 で動いていて、こちらは定義側でモデルが固定されているおかげでした。

この記事では、なぜ `general-purpose` が Opus で走っていたのかと、Haiku 固定のカスタム agent に切り出す判断軸の話をします。
...
クロス集計を見ると、`Explore` 列だけきれいに Haiku で揃っていて、それ以外は Opus が中心になっています。`general-purpose` は 12 件のうち 11 件が Opus で、Sonnet が混じった 1 件は親会話側で Sonnet を使っていた期間に起動したものでした。
...
`Explore` は、subagent の定義そのものが「read-only で動く検索専用、モデルは Haiku」と決まっています。Claude Code の `subagent_type: "Explore"` を指定して呼び出すと、親会話がどのモデルで動いていても子は Haiku 4.5 になります。15 件中 15 件が Haiku 4.5 で揃っていたのはそのためでした。
...
`Plan`、`general-purpose`、catch-all の `claude` は、定義側ではモデルを固定していません。親会話で使っているモデルが子にもそのまま渡ります。親が Opus 4.7 のときは子も Opus 4.7、Opus 4.8 に切り替えた期間は Opus 4.8、Sonnet を使っていた瞬間に起動した 1 件だけ Sonnet になっていました。

「親が Opus なら子も Opus でいいのでは」と思うかもしれませんが、問題は `general-purpose` に投げるタスクの種類です。私の場合、新機能の前提知識を渡したうえでの部分的な調査(ファイルを読んでまとめてもらう、依存関係を確認してもらう)を `general-purpose` に投げることが多いです。これは Haiku でも十分にこなせる粒度の作業ですが、親が Opus なら子も Opus が走ります。

「軽い調査も Opus で走る」は、こうやってデータを見るまで気付きませんでした。

## 計測する前と後で、判断の前提が変わった

カスタム agent を作ってこなかった理由を、自分のなかで言語化するとこうなります。
...
つまり「保守コストを払わない」という判断の裏側で、私は「親会話のモデルを継承するコスト」を払い続けていました。どちらのコストが重いかは、データを見てから決め直す段階にきています。

`Explore` がきれいに Haiku で揃っていたのは、私が意図して選んだ結果ではありません。subagent 定義側にそう書いてあっただけでした。同じ構造を `general-purpose` 相当の用途に対しても自分で再現できれば、判断を意識せずにモデル選択を縛れます。これがカスタム agent を作る動機になりました。
...
私の場合、「新機能を実装する前に、関連するファイル群と既存実装の方針を読んでまとめてもらう」というタスクが繰り返し出てきます。これを `general-purpose` に投げると、毎回 Opus が走ります。
...
Haiku 固定の調査専用 agent として切り出すのが妥当そうです。Haiku で品質が落ちないかは、結局のところ「判断不要、読みと要約のみ」の境界をどこに引くかで決まります。境界を超える依頼は親側で受けるか、Opus 系の agent に振り分ける形で運用するのがよさそうです。
