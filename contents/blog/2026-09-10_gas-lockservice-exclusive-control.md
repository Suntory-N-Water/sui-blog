...
社内の書籍を貸し借りするウェブアプリを、Google Apps Script とスプレッドシートで作りました。1 冊の本を同時に 2 人が借りられては困るので、`LockService.getScriptLock()` で排他制御をします。ここまでは一般的なやり方です。

ただし、ロックの取得と解放を書くだけでは足りません。二重の登録を止めているのは、貸出中かどうかの判定に使う値を、ロックを取得した後に読み直している点です。この記事では、実際にロックを競合させて観測した結果と、clasp 3.x で公開するまでの手順を書きます。

> [!NOTE]
> 確認したのは clasp 3.4.1、Apps Script の V8 ランタイム、rollup 4.63.1 の組み合わせです。clasp 2.x はコマンドの名前が違い、TypeScript の変換も本体が行っていました。
...
`openById()` を使うため、`appsscript.json` の `oauthScopes` に指定したスコープは `https://www.googleapis.com/auth/spreadsheets` です。スタンドアロンスクリプトには対象のスプレッドシートが決まっていないので、`spreadsheets.currentonly` では対象を認可できないと考えられます。ただし、`currentonly` に変更して失敗することは確かめていません。確認したのは `spreadsheets` で成功する側だけです。

## TypeScript を Apps Script から呼べる形にする

clasp 3.x は TypeScript を変換しません。README の [Migrating from 2.x to 3.x](https://github.com/google/clasp/blob/master/README.md#migrating-from-2x-to-3x) に、`Drop typescript support` という見出しで書かれています。
...
この指定を選ぶ理由は、Apps Script の側の制約にあります。Apps Script が呼び出せるのは、スクリプトの最上位にある `function` 宣言だけです。`doGet` もトリガーも `google.script.run` も、最上位に無い関数は見つけられません。rollup の `esm` 出力は、複数のモジュールを 1 つのスコープへ展開します。各モジュールの `export function` は、そのまま最上位の `function` 宣言になります。
...
この関数だけでは二重の更新は止まりません。結果を左右するのは、`lendBook` がどこで状態を読むかです。
...
`loans` に増えた未返却の行は 1 行で、`books.status` は `lent` になりました。2 回目が拒否されたのは、`findBookRow` が読み直した `status` が `lent` だったからです。この 2 つの呼び出しは同じ実行の中で連続して行われたもので、別々の実行が同時に入った状態ではありません。

### 状態を読む位置をロックの外へ出す

前の節で 2 回目が拒否されたのは、`findBookRow` がロックの中にあるからです。同じ処理で、読む位置だけをロックの外へ出した関数を用意しました。`lendBook` との違いは `findBookRow` の行だけです。
...
2 件目の書き込みは `withScriptLock` の中で行われています。ロックの取得は成功していました。それでも通ったのは、`commitLoan` が判定に使う `book.status` が、ロックを取る前に読んだ `available` のままだったからです。1 冊の本に、未返却の貸出が 2 行残りました。
...
ウェブアプリの実行者と公開範囲を決めるのは、`appsscript.json` の `webapp` だけです。
...
> [!WARNING]
> `access` を `ANYONE_ANONYMOUS` にすると、Google アカウントへのログイン無しで誰でも開ける状態が、CLI の実行だけで作られます。画面での確認は入りません。`appsscript.json` は、差分の確認を必須にする対象になります。
...
スプレッドシートを作る `setupSpreadsheet` は 1 回だけ実行します。`clasp run-function` で呼ぶつもりでしたが、失敗しました。
...
同じページの `Setup Instructions` には、これに加えて Apps Script エディタの Project Settings で Cloud プロジェクトの番号を設定する操作と、`appsscript.json` のスコープで認可するための `clasp login --use-project-scopes` が並んでいます。1 回だけ実行するセットアップ関数のためにこれを揃える理由は無いので、エディタから実行する方針にしました。この判断が、コードの書き方を 2 か所変えます。
...
clasp には MCP サーバ（`clasp start-mcp-server`）も付いています。3.4.1 の実装に登録されているツールは `push_files` / `pull_files` / `create_project` / `clone_project` / `list_projects` の 5 つで、バージョンの作成とデプロイは含まれません。この記事で使ったコマンドのうち MCP のツールに対応するものがあるのは、`create-script` と `push` の 2 つでした。`create-version` と `create-deployment` と `update-deployment` は、Bash から `clasp` を直接実行することになります。
