...
しかし、ロックの取得と解放を書くだけでは二重の更新を防げませんでした。二重の更新を防ぐには、貸出可能かどうかの判定に使う値を、ロックを取得した後にスプレッドシートから読み直す必要があります。ロックを取得する前に読んだ値を使って判定すると、ロックを取得していても二重の更新が実行されてしまいます。

この記事では、書籍貸出アプリを使い、ロックの外側で状態を読んだ場合に起きる競合の観測結果と、二重の更新を止めるコードの書き方を検証ログとともに解説します。排他制御に入る前に、競合が起こるデータの構造と、TypeScript を動かすためのビルド環境を先に整理したうえで、clasp 3.x によるデプロイ手順までをまとめます。
...
排他制御を確かめるために作った書籍貸出アプリの画面は、3 つの部分でできています。自分が借りている本の一覧、蔵書の一覧と絞り込み、管理者だけに表示される登録フォームです。どの処理で競合が起きるかを把握するため、まずは画面とスプレッドシートの構造を整理します。データの保存先はスプレッドシート 1 つで、`books` / `loans` / `config` の 3 シートに分けました。
