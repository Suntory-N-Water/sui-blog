...
行き着いたのが「振り返りのタイミングを決めない」という設計です。旅行先を考えていて過去のメモが気になったとき、技術調査をしていて以前似たことを調べた気がするとき、そういう「気になったタイミング」に検索するようにしています。
...
日々のメモは 2 つの方法で Obsidian に入ってきます。1つは [Thino](https://github.com/Quorafind/Obsidian-Thino) というプラグインで、Twitter のような投稿形式でその場に思いついたことを書けます。もう1つは LINE からの同期で、外出中に思いついたことを LINE で自分宛てに送ると、自動で Obsidian に記録されます。
