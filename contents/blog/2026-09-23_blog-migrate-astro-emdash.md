...
特に大きな理由はないのですが、Astro や Cloudflare のサービスを全面的に使おうとしたとき、SSG で静的なファイルを配信するだけでは全ての機能を活かしきれないと考えました。EmDash の成長を追いかけたいという思いもあり、今回の移行を決めました。
...
そこから紆余曲折あって、ブログの構成は特に手を入れずにそのままにしていました。ただ、AI がこれだけ開発に使われている中で、個人開発だけでなく複数人での開発や実務で AI ネイティブな開発を運用したときに、どういった部分が問題になるのかを学びたいと考えました。その題材として、Cloudflare ネイティブの CMS である EmDash を使うことにしました。EmDash の CLI と MCP サーバーから記事を書かせてみた内容は、2026 年 9 月の記事「[EmDash の CLI と MCP の両方で記事を書かせてみる](/blog/tried-emdash-cli-and-mcp)」にまとめています。
...
記事系のルートは `maxAge` を 300 秒(5 分)に留めています。Workers のキャッシュには、記事を公開したときに自動で破棄する仕組みがありません。そのため、有効期間を長くすると、更新が反映されるまでの時間も延びてしまいます。
...
EmDash にはこの値を変える設定項目がないため、`middleware.outer` に登録したミドルウェアでレスポンスヘッダーを上書きし、ブラウザで 1 時間キャッシュされるようにしました。

### 記事一覧と日本語フォントの読み込み量を減らす

記事一覧は全 130 件を 1 回のクエリで取得していました。EmDash の `getEmDashCollection` は取得する列を絞れず本文まで読み込むため、D1 から 1 リクエストあたり 3.19MB を転送しており、そのうち 2.97MB が本文でした。一覧を 1 ページ 20 件に分割して転送量を減らしています。
...
最終的には `objectCache` を外し、EmDash が D1 から直接読む構成に戻しました。リンクプレビューの保存先も KV から D1 に移しています。あわせて、コードブロックのハイライトや Mermaid 図を描画した結果の HTML も D1 に保存し、次のリクエストでは描画をやり直さずに再利用するようにしています。

## Cloudflare の Workers Paid プランで使えるサービス

ここからはブログの移行とは少し異なる話です。

Cloudflare では、月額 5 ドルからの Workers Paid プランに入るだけで、この価格でこんなにいろいろなことを試していいのかと思うほど多くのサービスを利用できます。今回使った Workers、D1、R2、KV もこのプランの範囲で使えています。
