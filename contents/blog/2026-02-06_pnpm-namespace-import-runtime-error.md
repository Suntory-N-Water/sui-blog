...
これらのパッケージはデータ変換処理やGoogle Cloud Storageへのアクセスといった基盤的な機能を提供していますが、npmを使い続ける中で依存関係管理に起因する課題が生じていました。
...
余談ですが、移行を進めている最中の2025年8月26日、Nx[^nx]において、攻撃者が悪意のあるバージョンを複数公開する事件が発生しました。
...
私が調べた限りでは、tsconfig.json の `esModuleInterop`[^esmoduleinterop] を有効にすれば解決するという情報を見かけましたが、今回の問題では解決されませんでした。
