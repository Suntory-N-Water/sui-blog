...
Node.jsのパッケージマネージャーは、2010年にnpmが登場して以来、フラットなnode_modules構造による高速化と互換性を重視してきました。しかし2017年ごろから、この設計が「ファントム依存」(package.jsonに記載していない依存パッケージを直接参照できてしまう問題)という課題を生むことが指摘され始め、同年これを解決するためpnpmが登場しました[^pnpm]。
...
[^pnpm]: 2017年にリリースされたパッケージマネージャー。npmのフラット構造によるファントム依存問題を、シンボリックリンクを用いた厳格な依存関係管理で解決する。
