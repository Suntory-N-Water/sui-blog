...
ログを見ると理由が分かります。`POST /_emdash/api/setup` が呼ばれた直後に画像のダウンロードとアップロードが実行されています。初期セットアップの完了が seed 適用のトリガになっている設計です。
...
CMS の都合上、毎回のリクエストで DB へのクエリが発行される設計になっています。そのためキャッシュが働いていないとき、画面表示で数秒待たされることがありました。
